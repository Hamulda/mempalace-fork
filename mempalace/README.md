...
| `searcher.py` | Semantic search via LanceDB vectors — filters by wing/room, returns verbatim + scores |
...
The palace (LanceDB) stores verbatim content. The knowledge graph (SQLite) stores structured relationships. The MCP server exposes both to any AI tool.
